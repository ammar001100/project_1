...
<li class="breadcrumb-item active"> تقرير مستخدم</li>
@endsection
@section('content')
			...
			************* -->
			<div class="main-container">
				...
				<div class="content-wrapper">
						...
						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
							<div class="card">
								<div class="card-body text-right">
								  	<h5>تقرير طلبات المستخدم : {{$employee->name}} </h5>
									<h5> مجموع السعر : @if(!empty($reports)) {{$reports->sum('total')}} @else 0 @endif </h5>
									<h5> عدد الطلبات : @if(!empty($reports)) {{$reports->count()}} @else 0 @endif </h5>
								</div>
							</div>
						</div>
					</div>
					<!-- Row end -->
                <!-- Row start -->
				<form  action="{{route('report.employee.order',$employee->id)}}" method="get">
					<div class="row gutters">
						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
							<div class="card">
								<div class="card-body">
									<div class="row gutters">
									    ...
									    <div class="col-xl-4 col-lg-4 col-md-4 col-sm-4 col-12">
											<div class="form-group">
												<label></label>
												<button type="submit" class="form-control btn btn-success" id="date-formatting2">بحث</button>
											</div>
										</div>
									   <div class="col-xl-4 col-lg-4 col-md-4 col-sm-4 col-12">
											<div class="form-group text-center">
												<label>الى</label>
												<input type="text" name="date_to" value="@if(isset($request)) {{$request['date_to']}} @endif" class="form-control input-group-text date-formatting" placeholder="YYYY-MM-DD" id="date-formatting" />
											</div>
										</div>
										<div class="col-xl-4 col-lg-4 col-md-4 col-sm-4 col-12">
											<div class="form-group text-center">
												<label>من</label>
												<input type="text" name="date_from" value="@if(isset($request)) {{$request['date_from']}} @endif" class="form-control input-group-text date-formatting" placeholder="YYYY-MM-DD" id="date-formatting" />
											</div>
										</div>
						...
						<div class="table-container text-center">
									...
									<div class="table-responsive">
										...
										@foreach ($reports as $index=>$report)
											<tr>
											<td> 
											@foreach($report->reportProducts as $product_quantity)	
											<?php $quantity = $product_quantity->product_quantity?> 
											@endforeach
											{{$quantity}} x {{ $report->reportProducts->count() }}
										    </td>
											  <td>{{ $report['total'] }}</td>
